...
if ($trackingId === '' || $code === '') {
    dotship_flash('warning', 'Tracking ID and OTP required');
...
if (dotship_verify_otp($trackingId, $code)) {
    ...
    if ($next !== null) {
        ...
        dotship_notify($shipment, 'status_update', 'Shipment advanced via OTP to ' . $next);
        dotship_flash('success', 'OTP verified. Shipment advanced to ' . ucfirst($next));
    } else {
        dotship_flash('info', 'Shipment already at final status');
    }
} else {
    dotship_flash('danger', 'Invalid or expired OTP');
}
